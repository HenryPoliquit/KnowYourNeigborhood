...
export default async function storesRoutes(fastify) {
  ...
  fastify.get('/', async () => {
    const { rows } = await fastify.pg.query(
      'SELECT * FROM kyn.stores ORDER BY created_at DESC'
    );
    return rows;
  });
  ...
  fastify.get('/:id', async (request, reply) => {
    const { rows } = await fastify.pg.query(
      'SELECT * FROM kyn.stores WHERE id = $1',
      [request.params.id]
    );
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const { rows } = await fastify.pg.query(
        `INSERT INTO kyn.stores (name, phone_number, locality, latitude, longitude, user_id)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`,
  ...
  fastify.delete(
    ...
    async (request, reply) => {
      ...
      const { rowCount } = await fastify.pg.query(
        'DELETE FROM kyn.stores WHERE id = $1 AND user_id = $2',
